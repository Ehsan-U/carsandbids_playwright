...
class CarsSpider(scrapy.Spider):
    ...
    # extract cookies and API endpoint from response
    ...
    def parse(self,response):    
        ...
        # handling pagination by specifying offset in url
        self.past_api = self.past_api.replace(f"offset={self.offset}",f"offset={self.offset+count}")
        if self.offset <= self.data['total']:
            self.offset+=count
            yield scrapy.Request(url=self.past_api,callback=self.parse,cookies=self.cookies)
